...
impl AnthropicAdapter {
    ...
    fn build_request_body(
        ...
        // - 非 off 档位：发送 output_config.effort 直接透传（官方推荐参数形态，替代废弃的
        //   budget_tokens 换算），并移除 temperature/top_p（与 extended thinking 行为一致）
        // - off/未设置：不发送 effort（模型默认 high），保留 temperature/top_p
...
mod tests {
    ...
    /// 测试设置 reasoning_effort=high 时，发送 output_config.effort 并跳过 temperature/top_p
    #[test]
    fn test_build_request_body_thinking_high() {
        ...
        // effort 直接透传到 output_config，不再换算 budget_tokens
        assert_eq!(body["output_config"]["effort"].as_str().unwrap(), "high");
    ...
    /// 测试 reasoning_effort=low 时，output_config.effort 原样透传
    #[test]
    fn test_build_request_body_thinking_low() {
        ...
        assert_eq!(body["output_config"]["effort"].as_str().unwrap(), "low");
    }

    /// 测试 reasoning_effort=max 时，output_config.effort 原样透传（不再依赖 max_tokens 换算）
    #[test]
    fn test_build_request_body_thinking_max() {
        ...
        assert_eq!(body["output_config"]["effort"].as_str().unwrap(), "max");
    }

    /// 测试 reasoning_effort=off 时，不发送 output_config，保留 temperature/top_p
    #[test]
    ...
    /// 测试未设置 reasoning_effort 时，请求体保持现有行为（无 output_config，含 temperature/top_p）
    #[test]
